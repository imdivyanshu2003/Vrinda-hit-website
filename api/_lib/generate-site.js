// ============================================================
//  VRINDA HIT — Shared site-generation helper
//  Callable from /api/generate (manual/test) and /api/trigger-generate
//  (invoked after Cashfree payment success).
//  Returns { slug, url, meta } or throws an Error with { code, detail }.
// ============================================================

import Anthropic from '@anthropic-ai/sdk';
import { getSupabase, SITES_BUCKET } from './supabase.js';
...
8. **Copywriting** must be specific to their actual idea — NOT generic placeholder text. Pull facts/claims from the idea brief. If the idea is vague, invent plausible specifics that match the niche.
...
function extractHtml(text) {
    ...
    if (end !== -1) return { html: cleaned.slice(start, end + '</html>'.length), truncated: false };
...
/**
 * Generate and persist a site. Returns { slug, url, meta }.
 * Throws Error with `.code` ∈ { validation, no_api_key, claude_empty, storage, db } on failure.
 */
export async function generateSite(input) {
    const { idea, brand, theme, style, palette, plan = 'premium', email, phone, orderId } = input || {};

    if (!idea || idea.length < 8) throw Object.assign(new Error('Missing or too-short idea'), { code: 'validation' });
    if (!THEMES[theme])    throw Object.assign(new Error(`Invalid theme: ${theme}`), { code: 'validation' });
    if (!STYLES[style])    throw Object.assign(new Error(`Invalid style: ${style}`), { code: 'validation' });
    if (!PALETTES[palette]) throw Object.assign(new Error(`Invalid palette: ${palette}`), { code: 'validation' });

    const apiKey = process.env.ANTHROPIC_API_KEY;
    if (!apiKey) throw Object.assign(new Error('ANTHROPIC_API_KEY not configured'), { code: 'no_api_key' });
    ...
    const message = await client.messages.create({
        ...
        max_tokens: 12000,
        temperature: 0.7,
        messages: [{ role: 'user', content: buildPrompt({ idea, brand, theme, style, palette, plan, email, phone }) }]
    });

    const rawText = (message.content || []).filter(b => b.type === 'text').map(b => b.text).join('\n');
    const extracted = extractHtml(rawText);
    if (!extracted) {
        const err = new Error('Claude response did not contain valid HTML');
        err.code = 'claude_empty';
        err.detail = { stopReason: message.stop_reason, preview: rawText.slice(0, 400) };
        throw err;
    }
    ...
    const { error: uploadErr } = await supabase.storage
        .from(SITES_BUCKET)
        .upload(storagePath, html, { contentType: 'text/html; charset=utf-8', upsert: false });
    if (uploadErr) {
    ...
    const safeOrderId = (typeof orderId === 'string' && UUID_RE.test(orderId)) ? orderId : null;

    const { error: insertErr } = await supabase.from('sites').insert({
        slug, order_id: safeOrderId,
        brand: brand || null, idea, theme, style, palette, plan,
        email: email || null, phone: phone || null,
        storage_path: storagePath, html_size_bytes: htmlSizeBytes,
        model: message.model, tokens_in: tokensIn, tokens_out: tokensOut, latency_ms: latencyMs
    });
    if (insertErr) {
        try { await supabase.storage.from(SITES_BUCKET).remove([storagePath]); } catch (_) {}
        const err = new Error('DB insert failed: ' + insertErr.message);
        err.code = 'db';
        throw err;
    }
